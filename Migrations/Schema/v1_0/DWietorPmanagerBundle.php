...
namespace DWietor\Bundle\PmanagerBundle\Migrations\Schema\v1_0;
...
class DWietorPmanagerBundle implements Migration
{
    ...
    {
        $table = $schema->createTable('d_wietor_pmanager_template');
        ...
        $table->addIndex(['user_owner_id'], 'IDX_dwietorEE2DB9449EB185F9', []);
        $table->addIndex(['organization_id'], 'IDX_dwietorEE2DB94432C8A3DE', []);
    ...
    /**
     * Create d_wietor_pmanager_translation table
     *
     * @param Schema $schema
     */
    protected function createPDFTemplateTranslationTable(Schema $schema)
    {
        $table = $schema->createTable('d_wietor_pmanager_translation');
        $table->addColumn('id', 'integer', ['autoincrement' => true]);
        ...
        $table->addColumn('content', 'text', ['notnull' => false]);
        $table->setPrimaryKey(['id']);
        $table->addUniqueIndex(['locale','object_id','field'], 'IDX_dwietorB520E901232D562BU');
        $table->addIndex(['object_id'], 'IDX_dwietorB520E901232D562B', []);
        $table = $schema->getTable('d_wietor_pmanager_translation');
        $table->addForeignKeyConstraint(
            $schema->getTable('d_wietor_pmanager_template'),
            ['object_id'],
            ['id'],
            ['onDelete' => 'CASCADE']
        );
