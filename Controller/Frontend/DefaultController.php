...
namespace DWietor\Bundle\PmanagerBundle\Controller\Frontend;
...
class DefaultController extends Controller {
    ...
    public function indexFrontendOrderAction(Order $order) {
        $info = $this->get('request')->get('d_wietor_pmanager_exportpdf');
        ...
        return $this->render(
            "DWietorPmanagerBundle:Default:indexfrontendorder.html.twig",
           $responseData
        );
    ...
    public function indexFrontendQuoteAction(Quote $quote) {
        ...
        return $this->render(
            "DWietorPmanagerBundle:Default:indexfrontendquote.html.twig",
           $responseData
        );
    ...
    protected function instancePDF($templateResult) {
        ...
        if(!is_null($header)){
            $resultForPDFHeader = $this->get('d_wietor_pmanager.pdftemplate_renderer')
        ...
        $pdfObj = $this->get("d_wietor_pmanager.tcpdf")->create($orientation, $unit, $format, true, 'UTF-8', false);
        if ($direction == 'rtl'):
            $pdfObj->setRTL(true);
        else:
            $pdfObj->setRTL(false);
    ...
    protected function getConfigurationProvider() {
        return $this->get('d_wietor_pmanager.provider.configuration');
    }
    protected function resultPDF($entityClass,$entityId,$process = "download",$isFrontend = false,$currentEntityId = 0){
        ...
        //$importForm = $this->createForm('d_wietor_pmanager_exportpdf');
        ...
        if ($templateResult = $this->get('d_wietor_pmanager.form.handler.exportpdf')->process()) {
            ...
            $pdfObj->AddPage();
            $outputFormat = 'pdf';
            $resultForPDF = $this->get('d_wietor_pmanager.pdftemplate_renderer')
            ...
            $templateResult = $this->getDoctrine()
                    ->getRepository("DWietor\\Bundle\\PmanagerBundle\\Entity\\PDFTemplate")
                    ->findOneBy(array('id' => $responseData['entityId']));
            $entity = $this->getDoctrine()
                    ->getRepository($responseData['entityClass'])
     ...
     * @Template("DWietorPmanagerBundle:Default:getTemplateFrontend.html.twig")
     */
    public function createFrontendOrderAction(Order $order) {
        ...
        if(!$configProvider->getEnableMultiTemplate() == 1){
            if($entityClass == "Oro\\Bundle\\OrderBundle\\Entity\\Order"){
                  if($configProvider->getDefaultOrderTemplate() != null){
                    ...
                    return $this->render(
                                "DWietorPmanagerBundle:Default:indexfrontendorder.html.twig", $responseData
                    );
                }
            }
        ...
        return $this->render(
            "DWietorPmanagerBundle:Default:getTemplateFrontendOrder.html.twig",
         array(
    ...
    public function createFrontendQuoteAction(Quote $quote) {
        ...
        if(!$configProvider->getEnableMultiTemplate() == 1){
            if($entityClass == "Oro\\Bundle\\SaleBundle\\Entity\\Quote"){
                  if($configProvider->getDefaultQuoteTemplate() != null){
                    ...
                    return $this->render(
                                "DWietorPmanagerBundle:Default:indexfrontendquote.html.twig", $responseData
                    );
                }
            }
        ...
        return $this->render(
            "DWietorPmanagerBundle:Default:getTemplateFrontendQuote.html.twig",
         array(
            'entityClass' => $entityClass,
            'entityId' => $entityId,
            'form' => $importForm->createView()
    ...
    protected function proccess(PDFTemplate $entity) {
        ...
        if ($this->get('d_wietor_pmanager.form.handler.exportpdf')->process($entity)) {
            $responseData['saved'] = true;
        }
        $responseData['form'] = $this->get('d_wietor_pmanager_exportpdf')->createView();
